...
//
//  -> complÃÂ©ter uniquement tsp_prog_dyn()
//  -> la structure "cell" est dÃÂ©finie dans "tsp_prog_dyn.h"
//
...
int ExtractPath(cell **D, int t, int S, int n, int *Q) {
  /*
    ...
    par tous les points de S, c'est-ÃÂ -dire le chemin extrait depuis la
  ...
  */
  if(D[0][1].length < 0) return 0; // si D n'a pas ÃÂ©tÃÂ© remplie

  // phase 1: extrait Q = V[t] ... V[n-1] dans cet ordre

  Q[0] = t;                   // ÃÂ©crit le dernier point V[t] en premier
  int k = 1;                  // k = taille de Q = nombre de points ÃÂ©crits dans Q
  while (Q[k - 1] != n - 1) { // on s'arrÃÂªte lorsque le point V[n-1] est atteint
    Q[k] = D[Q[k - 1]][S].pred;
    S = DeleteSet(S, Q[k - 1]);
    k++;
  }

  // phase 2: renverse Q pour avoir Q = V[n-1] ... V[t], au cas oÃÂ¹ la
  // fonction dist() ne soit pas symÃÂ©trique
...
double tsp_prog_dyn(point *V, int n, int *Q) {
  ...
  */
  ...
  for(int S = 1; S < C; ++S)
    for(int t = 0; t < L; t++)
    {
      int T = DeleteSet(S,t); //On evite de recalculer T a chaque fois
      if(T == S)
        continue;
    
    if(!(S&(S-1)))  //si card de S <=1 alors on a un seul point dans S, un seul choix
    {
      D[t][S].length = dist(V[n-1], V[t]); // D.lengh recois la distance entre le dernier point et le point t
      D[t][S].pred = n-1; //D.pred recois l'indice du dernier point
    }
    else //sinon on cherche le minimum
    {
      double min = DBL_MAX; //on initialise min a une valeur tres grande
      int min_pred = -1; //on initialise min_pred a -1 car on ne peut pas avoir d'indice negatif
      for(int x = 0; x < L; x++) //on parcourt tous les points
      {
        int X = DeleteSet(T,x);
        if(X==T){continue;} 

        if(D[x][T].length + dist(V[x], V[t]) < min) // si la distance entre x et t est plus petite que min alors on la remplace
        {
          min = D[x][T].length + dist(V[x], V[t]);
          min_pred = x;
        }
      }
      D[t][S].length = min; // on remet la valeur de min dans D.length
      D[t][S].pred = min_pred; // on met le chemin dans D.pred
